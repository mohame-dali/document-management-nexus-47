    ...
    >
      <TableCell className="font-medium text-right py-4">
        <div className="flex items-center gap-2.5 justify-end">
          <div className="text-right">
            <div className="font-bold text-base text-[#1a202c] group-hover:text-[#2c5282] transition-colors duration-200">
              #{doc.serialNumber}
            </div>
            <div className="text-sm text-[#718096]">{doc.year}</div>
          </div>
          <div className="flex-shrink-0 relative">
            {isIncomingDocument(doc) ? (
              <div className="p-2.5 rounded bg-[#ebf4ff] border border-[#bee3f8] text-[#2c5282] transition-colors duration-200">
                <FileInput className="h-5 w-5" />
              </div>
            ) : (
              <div className="p-2.5 rounded bg-[#ebf8f1] border border-[#bbf0d0] text-[#38a169] transition-colors duration-200">
            ...
            {activityUrgency && (
              <div className="absolute -top-1 -right-1 bg-white rounded-full p-0.5 shadow-xs">
                {activityUrgency.type === 'overdue' ? (
                  <AlertTriangle className="h-3.5 w-3.5 text-[#e53e3e]" />
                ) : (
      ...
      <TableCell className="text-right py-4">
        <div className="max-w-[280px]">
          <div className="font-medium text-base text-[#1a202c] line-clamp-2 mb-1.5 text-right leading-relaxed group-hover:text-[#2c5282] transition-colors duration-200">
            {doc.subject}
          </div>
          <div className="flex gap-1.5 flex-wrap justify-end">
            {doc.typeDocument && (
              <Badge variant="secondary" className="text-sm bg-[#f1f5f9] text-[#475569] border-[#e2e8f0] font-normal px-2.5 py-1 rounded">
                {doc.typeDocument}
              </Badge>
            )}
            {/* Activity urgency badge */}
            {activityUrgency && (
              <Badge className={`text-sm font-medium px-2.5 py-1 rounded ${activityUrgency.badgeColor} shadow-xs`}>
            ...
            {isIncomingDocument(doc) && doc.answer && (
              <Badge variant="outline" className="text-sm bg-[#ebf8f1] text-[#22543d] border-[#bbf0d0] font-medium px-2.5 py-1 rounded shadow-xs">
      ...
      <TableCell className="text-right py-4">
        <div className="inline-block text-sm font-medium text-[#2d3748] bg-[#f7fafc] px-3 py-1.5 rounded border border-[#e2e8f0]">
          {format(
      ...
      <TableCell className="text-right py-4">
        <div className="text-base max-w-[150px] truncate font-normal text-[#4a5568] group-hover:text-[#1a202c] transition-colors duration-200">
          {isIncomingDocument(doc) 
      ...
      {type === 'incoming' && (
        <TableCell className="text-right py-4">
          {isIncomingDocument(doc) && doc.activity ? (
            <div className="flex items-center gap-1.5 justify-end bg-[#ebf4ff] px-3 py-1.5 rounded border border-[#bee3f8] inline-flex">
              <Activity className="h-4 w-4 text-[#2c5282]" />
              <span className="text-sm truncate max-w-[120px] font-medium text-[#2c5282]" title={doc.activity}>
                {doc.activity}
              </span>
              {doc.dateActivity && (
                <span className="text-sm text-[#2c5282]/80">
                  ({format(new Date(doc.dateActivity), 'dd/MM')})
                </span>
              )}
            </div>
          ) : (
            <span className="text-[#a0aec0] text-base font-medium">-</span>
          )}
        </TableCell>
      )}
      ...
      {type === 'incoming' && (
        <TableCell className="text-right py-4">
          {isIncomingDocument(doc) && doc.responsibleUser && typeof doc.responsibleUser === 'object' ? (
            <div className="text-sm font-medium text-[#4a5568] bg-[#f7fafc] px-3 py-1.5 rounded border border-[#e2e8f0] truncate inline-block">
        ...
        <TableCell className="text-right py-4">
          {!isIncomingDocument(doc) && (doc as OutgoingDocument).assignedTo && (doc as OutgoingDocument).assignedTo.length > 0 ? (
            <div className="flex gap-1.5 justify-end">
              <Badge variant="outline" className="text-sm max-w-[100px] truncate bg-purple-50 text-purple-700 border-purple-200 font-medium px-2.5 py-1 rounded">
                {(doc as OutgoingDocument).assignedTo[0]}
              </Badge>
              {(doc as OutgoingDocument).assignedTo.length > 1 && (
                <Badge variant="outline" className="text-sm bg-purple-50 text-purple-700 border-purple-200 font-medium px-2.5 py-1 rounded">
                  +{(doc as OutgoingDocument).assignedTo.length - 1}
                </Badge>
              )}
            </div>
