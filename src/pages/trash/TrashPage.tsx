...
type FilterType = 'all' | 'incoming' | 'outgoing' | 'folder';
...
const TrashPage: React.FC = () => {
  ...
  const {
    ...
    queryFn: () => getTrashItems(filterType),
  });
  ...
  const filteredItems = items.filter((item) => {
    ...
    return labelMatch || idMatch || userMatch;
  });
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-4 sm:p-6 lg:p-8 space-y-6" dir="rtl">
      ...
      <div className="bg-white rounded border border-[#e2e8f0] p-4 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        {/* Type Tabs */}
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            className={`h-11 min-h-[44px] px-4 text-sm font-medium gap-1.5 ${
              filterType === 'incoming'
                ? 'bg-[#2c5282] hover:bg-[#2a4365] text-white'
                : 'text-slate-600 border-slate-200 hover:bg-slate-50'
            }`}
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-[#f7fafc] border-b border-[#e2e8f0] text-slate-600 text-xs font-bold uppercase tracking-wider">
                  <th className="py-4 px-6 text-right w-44">النوع</th>
                  <th className="py-4 px-6 text-right">الاسم / الموضوع</th>
                  <th className="py-4 px-6 text-right w-64">تاريخ الحذف</th>
                  <th className="py-4 px-6 text-right w-44">تم الحذف بواسطة</th>
                  <th className="py-4 px-6 text-center w-56">الإجراءات</th>
                </tr>
              </thead>
